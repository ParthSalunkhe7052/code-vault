...
def cmd_build(args):
    ...
    if getattr(args, "generic", False):
        ...
        color_print(
            "🔓 Generic Build Mode: License will be prompted at runtime", Colors.CYAN
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if success:
                copy_output(project_dir, config, license_key, args.output)
            else:
                color_print(
...
def run_pkg(project_dir: Path, config: dict) -> bool:
    ...
    cmd = [
        "npx",
        "pkg@5.8.1",
        str(project_dir / entry_file),
        "--targets",
        target,
        "--output",
        str(project_dir / output_name),
    ]
            ...
            if line_bytes:
                ...
                if line:
                    ...
                    if "error" in line.lower():
                        print(f"[NUITKA ERROR] {line}", flush=True)
                    elif "warning" in line.lower():
                        print(f"[NUITKA WARN] {line}", flush=True)
                    elif any(
                        kw in line.lower()
                        for kw in ["completed", "success", "done", "creating"]
                    ):
                        print(f"[NUITKA OK] {line}", flush=True)
                    else:
                        ...
                        if (
                            ...
                            print(f"[NUITKA] {line}", flush=True)

        process.wait()

        if process.returncode == 0:
            print(f"[NUITKA] Compilation completed successfully!", flush=True)
            return True
        else:
            print(
...
def copy_output(
    ...
    """Copy compiled output to current directory or custom path."""
    output_name = config["output_name"]
