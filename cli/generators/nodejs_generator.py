...
from templates.nodejs_tpl import (
    NODEJS_WRAPPER_LEGACY,
    NODEJS_WRAPPER_PREFIX,
    NODEJS_WRAPPER_SUFFIX,
)
...
def get_nodejs_wrapper_inline(
    ...
    safe_license_key = escape_for_js_string(validated_license_key)
    safe_server_url = escape_for_js_string(validated_server_url)
    safe_app_name = escape_for_js_string(app_name)
    ...
    prefix = prefix.replace("{heartbeat_interval}", str(heartbeat_interval * 1000))
    prefix = prefix.replace("{public_key}", public_key if public_key else "")
    prefix = prefix.replace("{app_name}", safe_app_name)
    prefix = prefix.replace("{binary_hash}", binary_hash if binary_hash else "skip")
