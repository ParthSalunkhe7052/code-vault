...
async def check_nuitka() -> Tuple[bool, str, Optional[str]]:
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            has_compiler = (
                "gcc" in output.lower()
                or "msvc" in output.lower()
                or "clang" in output.lower()
            )
            if not has_compiler:
                return (
                    True,
                    version,
                    "Nuitka installed but no C++ compiler detected. Install MinGW-w64 or MSVC.",
                )
            ...
            return (
                False,
                "Not working",
                "Nuitka returned error code. Try: pip install --upgrade nuitka",
            )
        ...
        return (
            False,
            "Not found",
            f"Nuitka not installed. Run: {sys.executable} -m pip install nuitka",
        )
        ...
        if process.returncode == 0:
            ...
            if not npx_path:
                return (
                    True,
                    version,
                    "Node.js installed but 'npx' not found. Check your PATH.",
                )
            return True, version, None
        else:
