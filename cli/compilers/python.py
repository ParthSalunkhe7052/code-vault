...
class PythonCompiler(BaseCompiler):
    ...
    async def compile(
        ...
        if fast_build:
            cmd.extend(
                [f"--jobs={max_jobs}", f"--output-dir={self.project_dir / 'build'}"]
            )
        else:
            cmd.extend(
                [
        ...
        try:
            ...
            if process.returncode == 0 and fast_build:
                self._create_launcher_script(output_name)
    ...
    def _create_launcher_script(self, output_name: str) -> None:
        """Create a launcher batch script for fast build output."""
        build_dir = self.project_dir / "build"

        exe_candidates = list(build_dir.rglob("*.exe"))
...
:: CodeVault Fast Build Launcher
:: This launcher runs the application from the build directory

set "SCRIPT_DIR=%~dp0"
set "BUILD_DIR=%SCRIPT_DIR%build\\{main_exe.parent.name}"
...
'''
        ...
        launcher_path = self.project_dir / f"{output_name}.bat"
