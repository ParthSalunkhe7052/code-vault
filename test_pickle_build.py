...
def test_build():
    ...
    try:
        ...
        cmd = [
            ...
            "--substitutions", f"_BUILD_ID={build_id},_SOURCE_URL={source_url},_CONFIG_URL=gs://codevault-builds/builds/{build_id}/config.json",
            "--format", "json", "--async"
        ]
        
        print(f"Running custom debug command...")
        result_proc = run(cmd, capture_output=True, text=True, shell=True)
