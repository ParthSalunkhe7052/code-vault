...
import { Loader2, CheckCircle, XCircle, Download, Monitor, Terminal, RefreshCw, X, RotateCcw, LucideIcon } from 'lucide-react';
...
interface MultiPlatformBuildStatusProps {
    buildId: string | null;
    autoRefresh?: boolean;
    onAllComplete?: (artifacts: Artifact[]) => void;
    onCancel?: () => void;
    onRetry?: (newBuildId: string) => void;
}

const MultiPlatformBuildStatus: React.FC<MultiPlatformBuildStatusProps> = ({ 
  buildId, 
  autoRefresh = true,
  onAllComplete,
  onCancel,
    ...
    try {
      ...
      if (!isCompleted || forceSync) {
        try {
          ...
          if (gcpData.status === 'completed' && gcpData.artifacts?.some((a: any) => a.download_url)) {
            ...
            const allDone = (gcpData.artifacts || []).every(
              (a: any) => a.status === 'completed' || a.status === 'failed' || a.status === 'cancelled'
            );
  ...
  const handleRetry = async () => {
    if (retrying || !buildId) return;
    setRetrying(true);
    try {
      const result = await cloudBuild.retry(buildId);
      onRetry?.(result.job_id);
    } catch (err: any) {
