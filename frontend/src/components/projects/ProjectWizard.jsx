...
import { Step1Upload, Step2Review, Step3Configure, Step4License, Step5Monetization, Step6Build } from './WizardSteps';
...
const ProjectWizard = ({
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            // PRIORITY 1: If there's an active or recent build, go straight to Step 6
            if (buildStatus === 'running' || buildStatus === 'pending') {
                // Active build - jump to build step immediately
                setCurrentStep(6);
                setCompletedSteps([1, 2, 3, 4, 5]); // Mark all previous steps as complete
    ...
    useEffect(() => {
        ...
        setShowQuickBuild(isConfigured && !isBuilding && currentStep < 6);
