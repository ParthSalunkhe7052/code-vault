...
/**
 * BuildHistory - Shows recent builds for a project with quick rebuild option
 * 
 * @param {string} projectId - The project ID to filter builds
 * @param {function} onRebuild - Callback when user wants to rebuild with same settings
 */
const BuildHistory = ({ projectId, onRebuild }) => {
    ...
    useEffect(() => {
        ...
        const fetchHistory = async () => {
            try {
                ...
                const projectBuilds = data.builds?.filter(b => b.project_id === projectId) || [];
    ...
    // Don't render anything if no projectId or still loading with no data
    if (!projectId) {
        return null;
    }
