...
const GlobalBuildStatus: React.FC = () => {
    const navigate = useNavigate();
    const { builds } = useBuild();

    const activeBuildEntry = Object.entries(builds).find(([_, b]) => 
        ['pending', 'queued', 'running'].includes(b.status)
    );
    
    if (!activeBuildEntry) return null;

    const [projectId, build] = activeBuildEntry;
