...
interface StatCardProps {
    ...
    value: string | number;
    icon: LucideIcon;
    color?: string; // Kept for compatibility even if not used in current version
    subtitle?: string;
    trend?: number;
}
...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon: Icon, subtitle, trend }) => (
    <div
        className="relative overflow-hidden rounded-2xl p-6 group transition-all duration-300
            shadow-lg shadow-black/20 hover:shadow-xl hover:shadow-black/30 border border-cv-border bg-cv-card-gradient h-full"
    >
        ...
        <div className="relative z-10">
            <div className="flex items-start justify-between mb-4">
                ...
                >
                    <Icon size={22} className="text-cv-primary" />
                </div>
                {trend !== undefined && (
