...
import { License, Project, CreateLicenseRequest } from '../types/api';
...
const Licenses: React.FC = () => {
    ...
    // Bindings logic (manual trigger/state for now to match original)
    ...
    const [newLicense, setNewLicense] = useState<CreateLicenseRequest>({
        project_id: '',
        client_name: '',
        client_email: '',
        max_machines: 1,
    ...
    useEffect(() => {
        if (projects && projects.length > 0 && !newLicense.project_id) {
            const firstProjectId = projects[0]?.id;
            if (firstProjectId) {
                setNewLicense(prev => ({ ...prev, project_id: firstProjectId }));
            }
        }
    }, [projects, newLicense.project_id]);
    ...
    // Get project name
    const getProjectName = (license: License) => {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        if ((newLicense.max_machines || 0) < 1) {
        ...
        try {
            ...
            await createMutation.mutateAsync(licenseData);
    ...
    const handleBulkRevoke = () => {
        confirm({
            ...
            onConfirm: async () => {
                ...
                await Promise.all(ids.map(id => revokeMutation.mutateAsync(id)));
    ...
    const handleViewBindings = async (license: License) => {
    ...
    const handleResetHwid = () => {
        ...
        confirm({
            ...
            onConfirm: async () => {
                setResetting(true);
                try {
                    const result = await licenseApi.resetHwid(selectedLicense.id, 'Admin manual reset');
                    toast.success(`HWID reset successful!`);
                    setBindings([]);
                    setResetStatus((prev: any) => prev ? {
                        ...prev,
                        current_bindings: 0,
