...
import { useDashboardStats, useProjects } from '../hooks/useDashboard';
...
const Dashboard: React.FC = () => {
    ...
    const validationSuccessRate = dashboardStats?.validations?.last_24h?.total 
        ? Math.round((dashboardStats.validations.last_24h.successful / dashboardStats.validations.last_24h.total) * 100)
        : 100;

    const projectCount = projectList.length || dashboardStats?.total_projects || 0;
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Alerts Section */}
            {dashboardStats?.expiring_soon && dashboardStats.expiring_soon.length > 0 && (
                <div className="glass-card p-6 border-amber-500/20 bg-amber-500/5">
                    ...
                    <div className="divide-y divide-cv-border-subtle">
                        {dashboardStats.expiring_soon.map((license: any, i: number) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-card p-6">
                    ...
                    {dashboardStats?.recent_activity && dashboardStats.recent_activity.length > 0 ? (
                        <div className="divide-y divide-cv-border-subtle">
                            {dashboardStats.recent_activity.slice(0, 6).map((activity: any, i: number) => (
