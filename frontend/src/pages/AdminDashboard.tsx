...
import {
    Users, Database, Key, Activity, RefreshCw,
    Shield, TrendingUp, Server, Globe, ArrowLeft, DollarSign,
    Zap, Crown, AlertTriangle, Search, Ban, CheckCircle
} from 'lucide-react';
...
const StatusIndicator: React.FC<{ status: string; size?: 'sm' | 'md' }> = ({ status, size = 'md' }) => {
...
const AdminDashboard: React.FC = () => {
    ...
    const [banConfirm, setBanConfirm] = useState<{ open: boolean; userId: string | null; email: string }>({ 
        open: false, userId: null, email: '' 
    });

    // Queries
    const { data: stats, isLoading: statsLoading, refetch: refetchStats, isFetching: statsFetching } = useAdminStats();
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-end justify-between border-b border-white/10 pb-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={handleRefresh} disabled={statsFetching} className="btn-secondary flex items-center gap-2">
                        <RefreshCw size={16} className={statsFetching ? 'animate-spin' : ''} />
                        Refresh
                    </button>
                </div>
            </div>
            ...
            {activeTab === 'overview' && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <StatCard
                            title="Success Rate"
                            value={stats?.total_compiles && stats.total_compiles > 0
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="glass-card p-6">
                            ...
                            <div className="h-64 flex items-center justify-center">
                                {revenue?.users_by_plan && revenue.users_by_plan.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <PieChart>
                                            ...
                                            >
                                                {revenue.users_by_plan.map((entry: any, index: number) => (
                        ...
                        <div className="glass-card p-6">
                            ...
                            <div className="space-y-4">
                                {revenue?.users_by_plan?.map((item: any) => (
                                    ...
                                    <tr key={user.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <select
                                                ...
                                                disabled={updateRoleMutation.isPending || user.role === 'banned'}
                                        ...
                                        <td className="p-4">
                                            <select
                                                ...
                                                disabled={updatePlanMutation.isPending || user.role === 'banned'}
                                                className={`px-2 py-1 text-xs font-medium rounded bg-slate-700 border border-slate-600 cursor-pointer ${
                                                    user.tier === 'business' ? 'text-amber-400' :
                                                    user.tier === 'pro' ? 'text-violet-400' : 'text-slate-300'
                                                } disabled:opacity-50`}
                                            >
                                        ...
                                        <td className="p-4 text-sm text-slate-300">{user.projects_count}</td>
                                        <td className="p-4 text-sm text-slate-300">{user.licenses_count}</td>
                                        <td className="p-4">
                                            {user.role !== 'banned' && (
                                                <button
                    ...
                    <div className="glass-card p-6">
                        ...
                        {systemHealth?.recent_errors && systemHealth.recent_errors.length > 0 ? (
                            <div className="space-y-2">
                                {systemHealth.recent_errors.map((error: any) => (
            ...
            {activeTab === 'webhooks' && (
                <div className="glass-card p-6">
                    ...
                    {analytics?.recent_webhooks && analytics.recent_webhooks.length > 0 ? (
                        <div className="space-y-2">
                            {analytics.recent_webhooks.map((webhook: any) => (
            ...
            <ConfirmDialog
                ...
                onClose={() => setBanConfirm({ open: false, userId: null, email: '' })}
                title="Ban User"
