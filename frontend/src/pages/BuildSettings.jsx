...
import { RotateCcw, Package, Clock, Server, Eye, Download, Bell, HardDrive } from 'lucide-react';
    ...
    return (
        <div className="p-6">
            ...
            <div className="max-w-2xl">
                ...
                <Section title="Output Strategy" icon={HardDrive}>
                    <Select
                        label="Compression Level"
                        description="Trade-off between build speed and file size"
                        value={settings.defaultCompression || 'standard'}
                        onChange={(v) => updateSetting('defaultCompression', v)}
                        options={[
                            { value: 'none', label: 'None (Fastest)' },
                            { value: 'standard', label: 'Standard (Balanced)' },
                            { value: 'max', label: 'Maximum (Smallest Size)' },
                        ]}
                    />
                    <Input
                        label="Filename Template"
                        description="Pattern for output files (Vars: {project}, {version}, {date})"
                        value={settings.defaultOutputPattern || '{project}_v{version}'}
                        onChange={(v) => updateSetting('defaultOutputPattern', v)}
                        placeholder="{project}_v{version}"
                    />
                    <Toggle
                        label="Clean Artifacts"
                        description="Automatically remove temporary build files after compilation"
                        value={settings.defaultCleanArtifacts !== false}
                        onChange={(v) => updateSetting('defaultCleanArtifacts', v)}
                    />
                </Section>

                <Section title="License Configuration" icon={Download}>
                     <Select
                ...
                <Section title="Notifications" icon={Bell}>
                    <Toggle
                        ...
                        description="Audible alert when build finishes successfully"
                        value={settings.playSoundOnComplete}
                        onChange={(v) => updateSetting('playSoundOnComplete', v)}
                    />
                    <Toggle
                        label="Persistent Toasts"
                        description="Keep success notifications until dismissed manually"
                        value={settings.persistentToasts}
                        onChange={(v) => updateSetting('persistentToasts', v)}
                    />
                </Section>
