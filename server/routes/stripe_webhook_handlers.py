...
async def handle_subscription_checkout_completed(session, conn):
    ...
    customer_id = (
        session.customer if hasattr(session, "customer") else session.get("customer")
    )
    subscription_id = (
    ...
    if not subscription_id:
        logger.warning(
            "[Stripe Webhook] checkout.session.completed without subscription_id, skipping"
        )
        return
    ...
    if existing:
        ...
        logger.info(
            f"[Stripe Webhook] Updated subscription {subscription_id} to tier: {tier}"
        )
    else:
        ...
        logger.info(
            f"[Stripe Webhook] Created subscription {subscription_id} for user {user_id}, tier: {tier}"
        )
...
async def handle_license_purchase_completed(session, conn):
    ...
    project = await conn.fetchrow(
        "SELECT id, name, user_id FROM projects WHERE id = $1", project_id
    )
    if not project:
    ...
    if existing_license:
        logger.info(
            f"[Stripe Webhook] License already exists for {buyer_email} on project {project_id}"
        )
        return
    ...
    """,
        ...
        buyer_email,
        f"Purchased via Stripe on {utc_now().isoformat()}",
    )
    ...
    logger.info(
        f"[Stripe Webhook] Created license {license_key} for {buyer_email} on project {project['name']}"
    )
...
async def handle_subscription_updated(subscription, conn):
    ...
    cancel_at_period_end = (
        ...
        else subscription.get("cancel_at_period_end")
    )
    ...
    if result:
        ...
        logger.info(
            f"[Stripe Webhook] Updated subscription {subscription_id}: status={status}, tier={tier}"
        )
...
async def handle_subscription_deleted(subscription, conn):
    ...
    if result:
        ...
        logger.info(
            f"[Stripe Webhook] Subscription {subscription_id} canceled, user downgraded to free"
        )
