...
async def init_database():
    ...
    try:
        ...
        # =============================================================================
        # Dodo Payments/Subscription Tables
        # =============================================================================
        ...
        await conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_license_purchases_dodo ON license_purchases(dodo_checkout_session_id)"
        )
        ...
        # =============================================================================
        # Migration 005: Tier Sync Improvements
        # =============================================================================
        try:
            await conn.execute(
                "ALTER TABLE subscriptions ADD COLUMN IF NOT EXISTS sync_source VARCHAR(20) DEFAULT 'dodo_webhook'"
            )
        ...
        # =============================================================================
        # Migration: Seller Infrastructure (Phase 1)
        # =============================================================================
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS sellers (
                user_id TEXT PRIMARY KEY REFERENCES users(id),
                dodo_vendor_id VARCHAR(255),
        ...
        # Marketplace columns for projects
        marketplace_columns = [
            ("projects", "is_public_store", "BOOLEAN DEFAULT FALSE"),
            ("projects", "price_cents", "INTEGER DEFAULT 0"),
            ("projects", "currency", "VARCHAR(3) DEFAULT 'USD'"),
            ("projects", "dodo_product_id", "VARCHAR(255)"),
            ("projects", "short_description", "VARCHAR(255)"),
            ("projects", "long_description", "TEXT"),
            ("projects", "cover_image_url", "TEXT"),
            ("projects", "category", "VARCHAR(50)"),
        ]

        for table, column, column_type in marketplace_columns:
            try:
                await conn.execute(
                    f"ALTER TABLE {table} ADD COLUMN IF NOT EXISTS {column} {column_type}"
                )
        ...
        # =============================================================================
        # Migration: Sales Table (Phase 2 - Buyer Storefront)
        # =============================================================================
        await conn.execute("""
        ...
        # =============================================================================
        # Migration: Marketplace Pivot (Phase 2 & 3)
        # =============================================================================
        ...
        # 2. Project Build Linking (The "Digital Good" Link)
        try:
            await conn.execute(
                "ALTER TABLE projects ADD COLUMN IF NOT EXISTS current_build_id TEXT REFERENCES cloud_builds(id) ON DELETE SET NULL"
            )
            print("[Migration] Added 'current_build_id' to projects table")
        except Exception:
            pass
            
        # 3. Standardize Payout Details in Sellers
        # (We already have payout_details JSONB, just ensuring it's flexible)
