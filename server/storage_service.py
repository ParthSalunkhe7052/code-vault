...
BUILD_RETENTION_DAYS = int(
    os.getenv("BUILD_RETENTION_DAYS", "30")
)  # Keep builds for 30 days
UPLOAD_RETENTION_DAYS = int(
    os.getenv("UPLOAD_RETENTION_DAYS", "90")
)  # Keep uploads for 90 days
...
def get_safe_project_dir(project_id: str) -> Path:
    ...
    if (
        not str(project_dir).startswith(str(base_resolved) + os.sep)
        and project_dir != base_resolved
    ):
...
class StorageService:
    ...
    def generate_download_url(
        self, key: str, filename: str, expires_in: int = 1800
    ) -> Optional[str]:
        """
...
async def cleanup_old_files() -> dict:
    ...
    stats = {"uploads_deleted": 0, "builds_deleted": 0, "bytes_freed": 0, "errors": []}
    ...
    print(
        f"[Storage Cleanup] Deleted {stats['uploads_deleted']} uploads, "
...
def validate_file_size(file_size: int, is_zip: bool = False) -> tuple[bool, str]:
    ...
    if file_size > limit:
        ...
        return (
            False,
            f"File size ({file_mb:.1f} MB) exceeds limit ({limit_mb:.0f} MB)",
        )
...
async def get_download_url(
    key: str, filename: str, is_local: bool = False
