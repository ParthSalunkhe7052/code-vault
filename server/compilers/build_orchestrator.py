...
from .nsis_builder import get_nsis_builder, NSISBuilder
...
class BuildOrchestrator:
    ...
    async def build_python_project(
        ...
        """
        await self.log(f"🐍 Building Python project: {config.project_name}", log_callback)
        ...
        try:
            ...
            if config.distribution_type == "installer":
                ...
                if not self.nsis_builder.is_available():
                    await self.log("⚠️ NSIS not available, returning portable exe", log_callback)
    ...
    async def _compile_python(
        ...
        # This defers to the existing lw_compiler module
        # For now, we'll use subprocess to call the CLI
        import subprocess
        
        output_name = f"{config.project_name}.exe"
        
        cmd = [
            "python", "-m", "cli.lw_compiler", "build",
            str(config.source_dir / config.entry_file),
            "--output", str(output_dir / output_name),
            "--api-url", config.api_url,
        ]
        
        if config.license_mode == "generic":
            cmd.append("--generic")
        elif config.license_mode == "demo":
            cmd.extend(["--demo", "--demo-duration", "60"])
        else:
            cmd.extend(["--license-key", config.license_key])
        
        await self.log(f"Running: {' '.join(cmd[:6])}...", log_callback)
        
        process = await asyncio.create_subprocess_exec(
            *cmd,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.STDOUT,
            cwd=str(config.source_dir.parent)
        )
        ...
        if process.returncode != 0:
            raise Exception(f"Nuitka compilation failed with code {process.returncode}")
    ...
    async def build_nodejs_project(
        ...
        2. Wrap in NSIS installer if requested
        """
        await self.log(f"📦 Building Node.js project: {config.project_name}", log_callback)
        ...
        try:
            ...
            exe_path = await compiler.compile(
                ...
                license_key=config.license_key if config.license_mode == "fixed" else "GENERIC_BUILD",
